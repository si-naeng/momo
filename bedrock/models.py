from mongoengine import Document, StringField, DictField, EmbeddedDocument, EmbeddedDocumentField
from mongoengine import fields, IntField
...
class MBTIEmotions(EmbeddedDocument):
    """
    MBTI 유형별 감정 데이터를 저장하는 임베디드 문서
    """
    emotions = DictField(default={})  # 감정: 횟수 형태로 저장 (예: {"기쁨": 3, "슬픔": 1})

class MBTIData(EmbeddedDocument):
    """
    MBTI 유형별 데이터를 저장하는 임베디드 문서
    """
    mbti_emotions = DictField(field=EmbeddedDocumentField(MBTIEmotions))  # MBTI 유형별 감정 데이터
